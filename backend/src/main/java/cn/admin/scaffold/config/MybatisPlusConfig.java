...
import cn.admin.scaffold.common.DataScopeInnerInterceptor;
...
public class MybatisPlusConfig {
    ...
    public static final Set<String> TENANT_TABLES = Set.of(
            ...
            "form_definition",
            "form_instance",
            // AI 知识库 / 提示词模板、消息模板、字段级审计日志
            // （上述表均带 tenant_id 列，必须纳入租户名单：漏加时 BaseMapper 标准 CRUD
            //  不注入租户条件，租户间存在越权读写风险）
            "ai_knowledge_base",
            "ai_knowledge_doc",
            "ai_prompt_template",
            ...
            "sys_field_audit_log",
            // 外部 MCP Server 配置（V41 新增表）须纳入租户名单：否则跨租户可列出/读写/删除
            // 他租户服务并携带其 authToken 连接调用外部工具，create 还会落到默认租户 1 造成数据污染
            "sys_mcp_server");

    /**
     * 带 tenant_id 却【有意不在】白名单的四张表——均已由服务层显式隔离，禁止误加：
     * <ul>
    ...
    /** 分页 pageSize 全局硬上限：客户端可传任意 pageSize 直达 SQL LIMIT，封顶防全表分页/OOM。 */
    public static final long MAX_PAGE_SIZE = 200L;

    @Bean
    public MybatisPlusInterceptor mybatisPlusInterceptor() {
        MybatisPlusInterceptor interceptor = new MybatisPlusInterceptor();
        // 拦截器顺序：条件注入类（租户/数据权限）必须在分页之前。
        ...
        // 分页 pageSize 全局封顶 200：客户端可传任意 pageSize 直达 SQL LIMIT，
        // 全部 37 处分页入口无上界时，?pageSize=100000000 可触发全表扫描拖垮 DB。
